-- Aerospace monitor sync. Held in a table the caller keeps alive: the
-- watcher, the running task and the pending timer are all
-- garbage-collected the moment nothing references them, and a collected
-- hs.screen.watcher stops firing without a word. See init.lua's global.
local M = {}
...
-- On the module table on purpose: a bare local would go out of scope when
-- this chunk returns, and nothing else holds the watcher — the callback
-- closure captures `pending` and `syncMonitors`, not the watcher itself.
-- The next collection would finalize it and screen changes would stop
-- arriving, silently and with nothing logged.
