...
1. Install GNU scripts, for better general tools:
...
1. Install the smarter cd command:
...
1. Install the modern replace for ls:
...
1. Install the cat replacement (with wings):
...
1. Install the corn for basic shell commands (the output colorizer):
   [`grc`][grc]
...
1. Install file colors config generator:
...
1. Install shell syntax highlightning:
...
1. Install suggestions as you type:
...
1. Install a better completion UI:
...
1. Install the fzf-tab binary module to speed up coloring: ([for
   reference][fzf-tab-bin-module]):

   ```shell
   build-fzf-tab-module
   ```

1. Install the lord of terminal multiplexers:
...
1. Install the way to load project sessions into it:
...
1. Install runtimes for `elixir`, `erlang`, `rust`, `python`, `node`, `yarn`,
   `just` and `zig`:
...
1. Install the worktree manager:
   [`worktrunk`](https://github.com/0k-software/worktrunk)
...
1. Install the `hs` command-line tool, so Hammerspoon can be driven from a
   shell instead of its GUI console. It is a symlink into the app bundle — the
   same one `hs.ipc.cliInstall()` would make from the console, just pointed at
   `~/.local/bin` (already on `PATH`) so no `sudo` is needed:

   ```shell
   ln -sf /Applications/Hammerspoon.app/Contents/Frameworks/hs/hs ~/.local/bin/hs
   ```

   It only answers once Hammerspoon is running a config that does
   `require("hs.ipc")` — `hammerspoon/init.lua` here does. Restart Hammerspoon
   after `make install` to pick that up, then check it:

   ```shell
   killall Hammerspoon; open -a Hammerspoon; sleep 2; hs -c 'hs.processInfo.version'
   ```

1. Make Hammerspoon the default browser, so links open in the Chrome window on
   the _focused_ aerospace workspace instead of wherever Chrome last had focus.
   macOS will ask for confirmation:
