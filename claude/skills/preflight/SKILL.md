...
written by step 1 right after its checks ran on that commit. `ship` reads it;
only preflight writes it.
...
**a. Code review.** Find the newest reviewed commit (see _Review marks_):
...
reviewed. Any later commit (a fix, an arrival) is not, and the next preflight
reviews it.
...
- **Changed** — step 4 committed anything (fixes, arrivals, checklist edits).
  The rebase and step 0's commit of a dirty tree do not count: step 1 reviewed
  ...
  you expect, run `<command>` again." Those commits are not reviewed yet; the
  next run reviews them.
...
- **Clear** — HEAD carries `reviewed`, every finding in the latest
