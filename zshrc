...
alias grm='git rebase $(git symbolic-ref refs/remotes/origin/HEAD 2>/dev/null | sed "s|refs/remotes/origin/||" || git branch -l main master | tr -d "* " | head -1)'
...
ai-jail-worktree() {
  local ssh_args=(--no-ssh --map "$HOME/.ssh/known_hosts" --map "$HOME/.ssh/config")
